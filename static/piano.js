/**
 * Piano — deux octaves zoomées (Do4 à Do6) plutôt que le clavier 88
 * touches complet : les notes de l'accord joué ressortent en couleur,
 * avec leur nom (fondamentale en bleu foncé, autres notes en bleu clair).
 */

const PIANO_NOTES     = ['C','C#','D','D#','E','F','F#','G','G#','A','A#','B'];
const PIANO_WHITE_SET = new Set([0, 2, 4, 5, 7, 9, 11]);

// Disposition de deux octaves complètes (MIDI 60..84 = Do4 à Do6) — Do4
// et Do6 sont tous deux des touches blanches, donc jamais de touche
// noire à moitié hors cadre en bord de clavier.
const PIANO_LAYOUT = (() => {
  const layout = [];
  let whiteCol = 0;
...
const PIANO_NUM_WHITE = 15; // Do à Do sur 2 octaves
...
function drawPiano(canvas, chord) {
  ...
  if (tones) {
    ...
    ctx.fillStyle = '#fff';
    for (const key of PIANO_LAYOUT) {
      if (!activeNcs.has(key.nc)) continue;

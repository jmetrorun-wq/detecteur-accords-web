...
class NetworkInterface:
    ...
    def __init__(self, net, save_name, load_checkpoint=False, load_path='cache_data'):
        ...
        if(os.path.exists(save_path)):
            # [ChordSplit] weights_only=False : les .sdict vendorés contiennent
            # aussi l'état de l'optimiseur Adam ; torch >= 2.6 met weights_only
            # à True par défaut et refuserait de les charger.
            state_dict=torch.load(save_path,map_location='cuda' if self.net.use_gpu else 'cpu',weights_only=False)
            ...
            state_dict=torch.load(cp_save_path,map_location='cuda' if self.net.use_gpu else 'cpu',weights_only=False)
            # The following codes are for torch 4.0 compatibility
            # new_state_dict={}
            # for key in state_dict['net']:
            #     if('num_batches_tracked' not in key):
